...
- 例外をキャッチした場合は `logger.error()` ではなく常に `logger.exception()` を使用すること
    - メッセージの中に例外オブジェクトを含めないこと: `logger.exception("Failed")` とする
- 可能な限り特定の例外を指定してキャッチすること
- `except Exception:` は厳禁（トップレベルのハンドラーは除く）
